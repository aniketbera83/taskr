...
function render() {
  ...
  if (isKanban) {
    ['todo', 'in_progress', 'done'].forEach(status => {
      ...
      document.getElementById('col-count-' + status).textContent = col.length;
      const list = document.getElementById('list-' + status);
      list.innerHTML = col.length ? col.map(taskCard).join('') : emptyState(status);
    });
  } else {
...
function taskCard(t) {
  ...
  const overdue = t.due_date && new Date(t.due_date) < new Date() && t.status !== 'done' ? ' overdue' : '';
  const cat = t.category_detail ? `<span class="badge badge-cat">${t.category_detail.name}</span>` : '';
  const dueHtml = due ? `<span class="due-badge${overdue}">${due}</span>` : '';
  return `
